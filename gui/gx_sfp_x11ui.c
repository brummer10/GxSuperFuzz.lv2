...
static void resize_event(gx_sfpUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_sfpUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_sfpUI* ui = (gx_sfpUI*)malloc(sizeof(gx_sfpUI));
	...
	ui->controls[0] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 30, 30, 71, 71}, false,"EXPANDER", KNOB, EXPANDER};
	ui->controls[1] = (gx_controller) {{ 0.0, 0.0, 0.0, 1.0, 1.0},{ 120, 30, 71, 71}, false,"TONE", SWITCH, TONE};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 200, 30, 71, 71}, false,"BALANCE", KNOB, BALANCE};
	ui->controls[3] = (gx_controller) {{ 0.25, 0.25, 0.0, 0.49, 0.01},{ 220, 330, 71, 71}, false,"TRIM", TRIMKNOB, TRIM};
	ui->block_event = -1;
	...
	ui->width = ui->init_width -140 + (55 * CONTROLS);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width-140 + (55 * CONTROLS));
...
static void knob_expose(gx_sfpUI *ui,gx_controller* knob) {
	...
	if (knob->type == TRIMKNOB) cairo_move_to (ui->crf, knobx1-extents.width/2, grow-64+extents.height);
	else cairo_move_to (ui->crf, knobx1-extents.width/2, grow+extents.height);
	cairo_show_text(ui->crf, knob->label);
	cairo_new_path (ui->crf);
}

// draw the power switch (bypass)
static void bypass_expose(gx_sfpUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void controller_expose(gx_sfpUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_sfpUI *ui, int controller) {
...
static void check_value_changed(gx_sfpUI *ui, int i, float* value) {
...
static void check_is_active(gx_sfpUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_sfpUI *ui) {
...
static bool get_active_ctl_num(gx_sfpUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_sfpUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_sfpUI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_sfpUI *ui, int i) {
...
static void button1_event(gx_sfpUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_sfpUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_sfpUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_sfpUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_sfpUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_sfpUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_sfpUI *ui, bool set) {
...
static void event_handler(gx_sfpUI *ui) {
	XEvent xev;
